...
// Inline the crowd sound clips so the single file works offline.
let jsWithAudio=js;for(const n of ['six','wicket','ooh','bed'])jsWithAudio=jsWithAudio.replaceAll(`assets/sfx/${n}.mp3`,`data:audio/mpeg;base64,${(await readFile(`assets/sfx/${n}.mp3`)).toString('base64')}`);
const html=(await readFile('index.html','utf8')).replace('<link rel="stylesheet" href="style.css">',()=>`<style>${css}</style>`).replace('<script type="module" src="app.js"></script>',()=>`<script type="module">${jsWithAudio.replaceAll('</script','<\\/script')}</script>`);
